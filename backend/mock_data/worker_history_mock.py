...
def get_worker(worker_id: str):
    for w in worker_db:
        if w["worker_id"] == worker_id:
            return w
    # Auto-register unknown IDs (e.g., Supabase UUIDs from React frontend)
    new_worker = {
        "worker_id": worker_id,
        ...
        "history_30d_orders": 150,
        "personal_30d_hourly_avg": 7.5,
        "past_claims_count": 1,
        "past_claims_valid": 1,
        "reliability_score": 85.0,
        "device_id": f"DEVICE_{worker_id[:8]}",
        "upi_id": f"{worker_id[:8]}@upi",
        "consent_weather": True,
        "consent_gps": True,
        "consent_orders": True,
        "flood_zone": 1,
    }
    worker_db.append(new_worker)
    return new_worker
